...
[x] plan-driven task: desktop shell에서 mock/replay local session source를 즉시 전환할 수 있게 만든다 [slice 87]
...
[x] plan-driven task: track start/sector marker를 board 위에 올려 2.5D 트랙 방향감을 높인다 [slice 95]
...
[x] plan-driven task: renderer-frame selected stale/no-telemetry 차량이 focus와 halo를 유지함을 직접 테스트로 고정한다 [slice 103]
[x] plan-driven task: renderer-frame fallback camera와 same-pulse halo attenuation 비교를 직접 테스트로 고정한다 [slice 104]
...
[x] plan-driven task: 2.5D 레이스 보드 MVP 를 완성한다 [slice 2]
[x] plan-driven task: 랩 단위 포디움 예측 AI 를 데스크톱 흐름 위에 단단하게 만든다 [slice 2]
[x] plan-driven task: 배포, 설치, 랜딩, 히스토리컬 데모를 포함한 공개 채널 준비를 마무리한다 [slice 2]

[x] plan-driven task: 로컬 앱 전환에 필요한 셸, 계약, 코어 분리 경계를 고정한다 [slice 3]
[x] plan-driven task: 2.5D 레이스 보드 MVP 를 완성한다 [slice 3]
[x] plan-driven task: 랩 단위 포디움 예측 AI 를 데스크톱 흐름 위에 단단하게 만든다 [slice 3]
[x] plan-driven task: 배포, 설치, 랜딩, 히스토리컬 데모를 포함한 공개 채널 준비를 마무리한다 [slice 3]

[x] plan-driven task: 로컬 앱 전환에 필요한 셸, 계약, 코어 분리 경계를 고정한다 [slice 4]
[x] plan-driven task: 2.5D 레이스 보드 MVP 를 완성한다 [slice 4]
[x] plan-driven task: desktop AI panel에 최근 예측 이력 strip을 추가해 lap 흐름을 바로 읽게 만든다 [slice 92]
[x] plan-driven task: 랩 단위 포디움 예측 AI 를 데스크톱 흐름 위에 단단하게 만든다 [slice 4]
[x] plan-driven task: 배포, 설치, 랜딩, 히스토리컬 데모를 포함한 공개 채널 준비를 마무리한다 [slice 4]

[x] plan-driven task: 로컬 앱 전환에 필요한 셸, 계약, 코어 분리 경계를 고정한다 [slice 5]
[x] plan-driven task: 2.5D 레이스 보드 MVP 를 완성한다 [slice 5]
[x] plan-driven task: 랩 단위 포디움 예측 AI 를 데스크톱 흐름 위에 단단하게 만든다 [slice 5]
[x] plan-driven task: 배포, 설치, 랜딩, 히스토리컬 데모를 포함한 공개 채널 준비를 마무리한다 [slice 5]

[x] plan-driven task: 로컬 앱 전환에 필요한 셸, 계약, 코어 분리 경계를 고정한다 [slice 6]
